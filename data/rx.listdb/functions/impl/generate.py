...
LEAF = (
    'execute if score $bit rx.temp matches @ if data storage rx:global listdb.entries[{bits:{b%:@b}}] run data modify storage rx:global listdb.entries[{bits:{b%:@b}}].bits.select set value 1b'  # noqa: E501
)
...
def gen_tree(bit_num, low, high):
    ...
    if low + 4 < high:
        ...
        make_file(Path(f'select/bit{bit_num}/{low}_{high-1}.mcfunction'), tree)
        ...
        make_file(Path(f'select/bit{bit_num}/{low}_{low+3}.mcfunction'), leaf)
